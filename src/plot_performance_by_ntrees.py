...
for i, k in enumerate(k_list):
        ...
        for n_tree in n_tree_list:
                lse_filename = _LARGE_SPREAD_FILENAME.format(dataset, dataset, _PARTITION_POLICY, n_tree, depth, k,round, random_state, m, s, min_perts[i], max_perts[i])
                print("EXAMINING ", lse_filename)
...
for i, k in enumerate(k_list):
        ax1.plot(n_tree_list, times[i], marker='o', ms=10, mew=4, mfc="w", label="LSE $k = {}$".format(_PERT_LABELS[k]))

if dataset == "mnist26":
        ax1.set_ylim(bottom=-5)
        ax1.set_ylim(top=150)
elif dataset == "fashion_mnist0-3":
        ...
        ax1.set_ylim(top=1930)

ax1.set_xlabel("#trees", fontsize=18)
...
ax1.set_title("depth={}".format(depth), loc='right', fontsize=18)
